...
const createPostFormRules = {
    ...
    slug: [
        {required: true, message: 'Please enter the slug', trigger: 'blur'},
        {
            ...
            message: 'Slug should be in kebab-case (i.e., lowercase alphanumeric characters separated by single hyphens)',
            trigger: 'blur'
        },
...
// Edit post modal
const showEditPostModal = ref(false), editPostTitle = ref(""), editPostContent = ref(''), editPostSlug = ref(''),
    editPostAuthorId = ref(''), editPostId = ref<number>(-1)
...
<template>
    <div class="w-full h-full">
            ...
            <NCard class="w-full flex m-3 p-2">
                <div>
                    ...
                    <div class="flex gap-2 w-full justify-end">
                        <NButton strong secondary type="default" @click="showCreatePostModel=false" size="large"
                                 :disabled="awaitPublishingPost">Cancel
                        </NButton>
                        <NButton strong primary type="primary" size="large" :loading="awaitPublishingPost"
                                 @click="tryPublishPost()" :disabled="awaitPublishingPost">Publish
                        </NButton>
                    </div>
                </div>
            </NCard>
        ...
        >
            <div class="mb-3">Are you sure you want to delete these Posts? This action cannot be reversed and these
                posts will be deleted permanently.
            </div>
            <div class="gap-3 flex align-bottom justify-end">
                <NButton strong secondary type="default" @click="onNegativeClick" :disabled="deletingPosts">Cancel
                </NButton>
                <NButton strong type="error" @click="onPositiveClick" :loading="deletingPosts"
                         :disabled="deletingPosts">Delete
                </NButton>
            </div>
        </NModal>
        <div class="w-full flex">
            ...
            <NButtonGroup class="items-center">
                ...
                <NButton @click="showConfirmDeleteModal=true" :disabled="checkedRowKeysRef.length == 0" strong secondary
                         type="error">Delete
                </NButton>
            </NButtonGroup>
        </div>
        ...
        <div class="w-full h-full" v-if="!loadingPage">
            <NScrollbar>
                <NDataTable ref="dataTable" :columns="columns" :data="data" :row-key="rowKey"
                            @update:checked-row-keys="handleCheck" @update-page="reloadPage"/>
            </NScrollbar>
        </div>
